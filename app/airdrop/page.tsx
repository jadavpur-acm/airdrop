...
import { Wallet, CheckCircle2, AlertCircle, Loader2, Mail, Phone, Image as ImageIcon, KeyRound, ArrowRight, Unplug, Users, Upload } from 'lucide-react'
...
export default function AirdropPage() {
        ...
        try {
            ...
            try {
                await axios.post('/api/check-eligibility', { email: formData.email });
                 ...
                 throw new Error(checkErr.response?.data?.error || "Eligibility check failed.");
            }
    ...
    const handleOtpSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.error(err)
            const msg = err.response?.data?.error || err.message || 'Invalid OTP or Registration Failed.'
            if (msg.includes("No team registration") || msg.includes("already minted")) {
                 setStep('REJECTED')
                 setError(msg)
            } else {
                setError(msg)
        ...
        <div className="min-h-screen text-white selection:bg-primary/30 pt-16 pb-12 px-4 relative overflow-hidden font-sans bg-black">
            ...
            <div className="max-w-xl mx-auto relative z-10 space-y-8">
                ...
                <div className="text-center space-y-4">
                    <Badge variant="outline" className="px-5 py-2 border-primary/30 bg-primary/10 text-white backdrop-blur-md rounded-full text-[10px] font-black tracking-widest uppercase">
                ...
                <Card className="border-white/10 bg-neutral-900/60 backdrop-blur-2xl shadow-2xl relative overflow-hidden group rounded-[2.5rem]">
                    ...
                    <CardHeader className="pt-10 px-8 text-center sm:text-left">
                        <CardTitle className="flex items-center gap-4 text-3xl font-black italic tracking-tight text-white">
                            {step === 'WALLET' && <><Wallet className="w-7 h-7" /> INITIALIZE WALLET</>}
                            {step === 'FORM' && <><ImageIcon className="w-7 h-7" /> TEAM REGISTRATION</>}
                            {step === 'OTP' && <><KeyRound className="w-7 h-7" /> SECURITY PASS</>}
                    ...
                    <CardContent className="pb-10 px-8">
                        {error && (
                            <div className="mb-6">
                                <Alert variant="destructive" className="bg-red-500/10 border-red-500/20 text-red-500 rounded-2xl p-4">
                                    ...
                                    <div className="ml-2">
                                        <AlertTitle className="text-xs font-black uppercase tracking-widest underline decoration-2 underline-offset-4">Error_Log</AlertTitle>
                            ...
                            <form onSubmit={handleFormSubmit} className="space-y-8">
                                <div className="space-y-5">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                        <div className="group space-y-2.5">
                                            <Label className="text-[11px] font-bold uppercase tracking-[0.2em] text-neutral-400">
                                                <Mail className="w-3.5 h-3.5 inline mr-2" /> Registered Email
                                            </Label>
                                            <Input
                                                type="email"
                                        ...
                                        <div className="group space-y-2.5">
                                            <Label className="text-[11px] font-bold uppercase tracking-[0.2em] text-neutral-400">
                                                <Phone className="w-3.5 h-3.5 inline mr-2" /> Verification Phone
                                            </Label>
                                            <Input
                                                type="tel"
                                    ...
                                    <div className="group space-y-2.5">
                                        <Label className="text-[11px] font-bold uppercase tracking-[0.2em] text-neutral-400 flex justify-between">
                                            <span><ImageIcon className="w-3.5 h-3.5 inline mr-2" /> Team Logo Artifact</span>
                                        ...
                                        <div className="relative group/upload">
                                            ...
                                            >
                                                {imagePreview ? (
                                                    <img src={imagePreview} className="h-full object-contain rounded-xl" alt="Preview" />
                                                ) : (
                                                    <div className="flex flex-col items-center gap-2 text-center px-4">
                                                        <Upload className="w-6 h-6 text-neutral-600" />
                                                        <span className="text-[10px] font-black text-neutral-600 uppercase">Select Image Asset</span>
                                                    </div>
                                                )}
                                            </label>
                                        </div>
                                ...
                                <Button type="submit" disabled={loading} className='w-full h-18 rounded-2xl font-black bg-white text-black hover:bg-neutral-200 text-base tracking-widest' size="lg">
                                     ...
                                     <div className="relative h-full flex flex-col p-8 z-10">
                                        ...
                                        <div className="flex-1 flex flex-col items-center justify-center gap-6">
                                            ...
                                            <div className="text-center space-y-1">
                                                ...
                                                <h3 className="text-2xl font-black italic tracking-tighter text-white uppercase">{formData.teamName}</h3>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        {step === 'REJECTED' && (
                            <div className="text-center space-y-10 py-8">
                                ...
                                <Button className="w-full h-16 rounded-2xl bg-white text-black hover:bg-neutral-200 font-black uppercase tracking-widest text-sm" onClick={() => setStep('FORM')}>
                                    RE-TRY REGISTRATION
                                </Button>
                            </div>
                        )}
